...
Use `--help` for the command summary. Use `--load PATH` to validate and restore a save before the interactive prompt; malformed or unreadable saves exit with an error and never start a session. The startup path is a single argument, like the in-game `load PATH` command.
